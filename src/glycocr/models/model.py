"""PyTorch model wrapper for PaliGemma 2 VLM with PEFT/LoRA adapter tuning."""
...
class GlycOCRModel(nn.Module):
    """PaliGemma 2 VLM wrapper with PEFT/LoRA adapters targeting attention layers."""

    def __init__(
        self,
        model_name: str = "google/paligemma2-3b-pt-448",
        lora_r: int = 8,
        lora_alpha: int = 16,
        target_modules: list[str] | Sequence[str] | None = None,
        device: str | torch.device | None = None,
    ) -> None:
        """Initialize PaliGemma 2 model architecture and apply LoRA configuration."""
        super().__init__()
        ...
        self.target_modules = (
            list(target_modules) if target_modules is not None else ["q_proj", "v_proj", "k_proj", "o_proj"]
        )

        self.processor = AutoProcessor.from_pretrained(model_name)
        base_model = PaliGemmaForConditionalGeneration.from_pretrained(model_name)
    ...
    def from_pretrained(
        ...
        instance.target_modules = list(getattr(default_cfg, "target_modules", ["q_proj", "v_proj", "k_proj", "o_proj"]))
